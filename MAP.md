...
- `lie_down` / `say` MCP (`daemon.py`, cortex-only) — registered only when `MARROW_CORTEX` set in the daemon's env at import (cortex window spawns `claude` with it; normal sessions never see the tools). Each runs `[cortex].venv_python -m cortex.lie_down|say` with `cwd=[cortex].repo_root` so the package resolves from any cwd (cortex venv has no editable install); 30s timeout, stderr surfaced on failure; empty config keys → "not configured" error. Replaced the old /lie-down + /say slash commands (07-08).
