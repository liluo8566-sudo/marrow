...
- PreToolUse `hooks:pretool_use` — Write/Bash placement ops get atlas ancestor-chain guidance (desc + naming_hint); others get a literal path reminder. Matcher=Agent → `hooks:agent_guard` — denies any Agent dispatch whose subagent_type is in `[agent_guard].deny` (default `["general-purpose"]`, burst-recursion protection); exit 2 + stderr reason, fail-soft (any error → exit 0). Backup guard, three tiers, fail-open (07-05/06): tier S silent (tmp/scratchpad/worktree rm/mv, read-only git) · tier 2 additionalContext reminder, once per session (git reset --hard/clean -f/push --force/branch -D/stash drop, DROP TABLE/TRUNCATE/unwhered DELETE, destructive MCP actions incl. event_clear/db_clear/sticker delete) · tier 3 backup-state gate on config paths (.claude.json/settings*.json/mcp*.json/.obsidian) — denies until a backup-ish command (cp/rsync/tar/git commit/git stash push) landed within `backup_guard_window_min`.
...
- `goal` MCP (`daemon.py`, action=set/list/delete) — key/value/unit upsert into `goals` (v30), no history. Any session calls `goal(action='set', ...)` the moment she states/changes a goal ("sleep goal 8h") — no file edit, next cortex tick reads it straight from DB.
- `wish` MCP (single `text` param, no action — always append) — append-only line into cortex's wishlist.md (`[cortex].wishlist_path`, default `<home>/wishlist.md`), flock-guarded atomic write, date-prefixed. Her hand edits in the md are never touched/rewritten — pure one-way DB-writer → md, no reconcile. No DB table (a wishes table shipped+reverted same day, 07-05).
- `first` MCP (action=tick/untick/list, status=done|tried) — main session's response to the Cortex First section (nudges injected into context, §3). 'tick' writes `ct_first_tick` (v32; +status v34) so other sessions/later wakes stop repeat-nagging the same item; 'untick' clears a wrong ack.
