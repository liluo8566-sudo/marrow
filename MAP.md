...
- Strong-hit: full-table scan, `recall:_expand_needles` tiered — name/aliases/key/title cjk 2-4 char windows (2-char CN names below the trigram floor); fact/value/description cjk 3-4 only (generic bigram guard). ASCII needles letter-boundary via `recall:_needles_match` — digits don't break, so (gpt) hits (gpt4画画) but (nd) can't hit inside (handover). Entity force-include lives HERE, in recall.py; entity_recall.py only does mention-count bumps.
