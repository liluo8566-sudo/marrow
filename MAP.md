...
- `wish(text)` — single-param, always append. Flock-guarded atomic write into `[cortex].wishlist_path` (default `<home>/wishlist.md`), date-prefixed line. Her hand edits in the md are never touched — one-way DB-writer → md, no reconcile, no DB table.
