...
- Any text/format/prompt/template that will be seen by Lumi or Stellen should be checked by Lumi before write in.
      e.g. affect tags, subagent prompt for dairy, handover template
...
- memes — private inside-jokes + viral memes Lumi quotes + topical news/event mentions; type: meme | cipher | news | event_mention; hot vocabulary first (table: `memes`)
- stickers — visual sticker assets, independent module (WeChat-sticker-style gallery): asset_path + description + trigger phrase; optional FK to memes if riding a known meme, else stands alone
...
Phase-2 / later: affect (per-episode, see DECISIONS) / entities+entity_facts (emitted in the single call, see DECISIONS; backs Profile sub-page when render lands) / corrections (placeholder) / transactions (FUTURE stellan_wallet; backs Wallet sub-page); emotions/people/preferences/dir placeholders removed.
...
- `memory/reference.md` <cipher> → memes (type=cipher)
...
- Profile — non-CLAUDE.md personal facts: interests, lifestyle preferences, family & friends. Backed by entities (Phase 2)
- Milestone — life events (## Us + ## Me)
- Diary — one sub-page per month, drill into per-day narrative
- Memes — private inside-jokes + viral memes Lumi quotes + topical news/event mentions; hot vocabulary first, name pending
- Stickers — WeChat-sticker-style gallery, bidirectional: drop a file in → system auto-writes description + trigger from chat context; remove from md or via chat command → gone. Reference: cyberboss sticker mgmt
...
- Wallet — Phase-2 opt-in addon, transactions table, bank-statement layout (see FUTURE stellan_wallet)
...
Principle: every sub-page is populated from two directions. The system grows the page without Lumi lifting a finger; Lumi can override any row by hand at any time. Monitor Zone (audit-log mirror) is read-only — see Dashboard section.

### System → md (auto-population)

Pipeline scans new events and writes candidate rows to sub-page tables: memes mentions, sticker uses, profile facts (interest / pref / person), news / event quotes, wallet spend leads. Hits land in the target table; next render reflects them on the sub-page. Idempotent on source_hash — same event scanned twice never double-inserts. Extraction channel (which LLM tier, which hook, whether folded into the existing single sonnet call or its own pass), confidence gate, and per-table trigger all Pending — settled per sub-page at build.

### md → System (manual correction)
...
- Structured views (Open Threads, milestone, memes, stickers, profile, pit, wallet, alerts): each row carries a visible short id at line/block end. id present + text changed → update; id deleted with the row → delete/abandon; new block with no id → insert. md edit is the primary path here.
...
The reconcile semantics above are fixed, not Pending. Only the anchor's character format + per-view render template are Pending (set when each view is built).

### Anti-revive guard

A row Lumi deletes via md or `mw` must not be re-inserted by the next auto extraction. Deletion path writes the row's source_hash (or content-hash) to a tombstone set; extractor skips matching hashes. Tombstone retention and key format Pending.

### Conflict guard

Hash-compare before any system overwrite. If Lumi changed the md, back up the old file and raise one Alert. Never overwrite in silence.
...
- Phase 2 — Emotion (affect) + decay + sub-page render fills out; entity (people/pref) emitted in the single call — see DECISIONS. Sub-page auto-population for memes / stickers / profile / news arrives with the corresponding sub-page render (extraction channel Pending). Sub-page render config-driven (goal 7); stellan_wallet first opt-in addon.
