...
- **stellan_wallet** — Opt-in dashboard addon: monthly allowance auto-credit + spend auto-debit. `transactions` table only, balance = SUM(amount) never stored; bank-statement sub-page, month-grouped. Auto-credit piggybacks the diary 04:00 routine (idempotent per month, catchup-safe, no new launchd job); spend detection rides the existing haiku-lessons extraction (one call, second sink: lessons[] + transactions[]); `mw set`/`rm` + md reconcile = correction path, no new code. Risk: per-session keep-soul digest may drop a small spend before merge — Phase 2 resolves by digest retaining spend leads or sinking extraction into the per-session map step; prompt body needs Lumi review (DESIGN L53). First opt-in addon, exercises the config-driven selectable sub-page contract. Render anchor: top `Balance` figure (= SUM, computed not stored) then a month-grouped bank-statement table, columns Date / Type (Credit|Debit) / Description / Amount (signed) / running Balance-after; row-tail short id for structured-view reconcile (source: Lumi 2026-05-18)
- **lesson_extraction_rework** — Removed from diary.py 2026-05-18 (LESSONS_PROMPT + run_day block + lessons table writes). Old impl over-extracted facts / tool discoveries / design decisions that already live in events + DESIGN/ADR. Re-add only as behavioural-failure-mode lessons: recurring "how I work" corrections the user made (interaction / prompt / coding habit / verification discipline); exclude anything stated once and findable in events. Also fix the non-idempotent re-insert on rerun. `lessons` table kept in schema, unused until redesign. NOTE: stellan_wallet's "spend detection rides haiku-lessons extraction" is now void — wallet needs its own spend-detection path. (source: Lumi 2026-05-18)
