...
- **dashboard_v2_redo** — Full dashboard top-block redesign once entry/storage flow is stable. Includes: (a) milestone candidate → pinned confirm button (no CLI, click-to-pin in-place); (b) task / candidate delete button (drop without manual SQL); (c) subpage cross-link anchors; (d) format unification across all top sections. Owner Lumi; format spec pending real data flowing in. (Lumi 2026-05-24)
